...
export function OutreachAdmin() {
  ...
  async function importSelected() {
    ...
    if (result?.contacts) setSelectedContactIds(new Set((result.contacts as Contact[]).filter((contact) => contact.status === "active").slice(0, 20).map((contact) => contact.id)));
  }
  ...
  return <div className="mt-10 grid gap-6">
    ...
    <Card className="p-6">
      ...
      <p className="mt-2 text-sm text-muted-foreground">Choose up to 20 active contacts below. Production sending remains locked until explicitly enabled.</p>
